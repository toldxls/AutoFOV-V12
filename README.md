...
V12.4 adds an **MTF / resolution analyzer** — drop one or more USAF-1951 target shots on the dashboard and it measures the system's slanted-edge MTF, plots it against the objective's diffraction limit, fits the **equivalent NA** the system actually performs at, and reports MTF50 / MTF10 / MTF5 in lp/mm or µm. Accepts JPEG, PNG, and baseline TIFF; scores multiple shots for focus and picks the sharpest automatically. Late in the 12.4 series the firmware also gained a **field-resilience layer** — boot-loop auto-rollback to the previous OTA slot, a reset-reason diagnostic ring (`/diag` + dashboard screen), a task watchdog with I²C-bus self-heal, calibration backup/auto-restore in NVS, and a wrong-password → captive-portal fallback — plus PSRAM-buffered OTA uploads that eliminated the old mid-upload stall.

V12.5 adds a **deep per-stack vibration history** for frame culling. During every stack the firmware records each frame's during-exposure blur (V/H, stage-µm) plus a full-stack RMS envelope in PSRAM; when the stack completes, the dashboard opens a **Stack Vibration Report** — every photo listed with its number, time, wall-clock, blur, and pixel-equivalent softness, ranked worst-first, with an adjustable flag threshold (default 2 px), a timeline strip with click-to-locate frame markers, a one-click **cull list** to the clipboard, and the stack-summary PNG card. The browser archives the last 8 reports, so they survive new stacks, page reloads, and device resets. Times come from frame numbers + device-relative clocks mapped in the browser (with an SNTP cross-check when the network allows) — no RTC needed.

V12.6 makes the distance **trustworthy across a day and a reboot**. Two findings drove it: the VL53L4CX's zero point re-references every time the sensor restarts, and its range drifts with room temperature at about **+1.3 mm/°C** (measured r = 0.97 over a 6 °C night — the "intrinsic silicon random walk" of earlier notes was mostly an unmeasured thermometer). So the firmware now has **session zero** — SET HOME once after an FOV calibration with the bellows racked to max, then **ZERO @ MAX RACK** at the start of each session (and after any re-lock, reboot or sleep/wake; it refuses while the sensor is still cold) — and **temperature compensation** fed by a passive 24 h log: the device records (IMU temperature, level) every 5 min, the dashboard fits the mm/°C coefficient and applies it only when a new fit strictly beats the last one's pedigree. The trend ring is checkpointed to flash hourly and before every deliberate restart, so an overnight soak survives the morning OTA (the graph marks each boot with a dashed seam). A **TOF TARGETS** diagnostics panel exposes the raw multi-target histogram, the SPAD servo count (the real brightness gauge — reported MCps is pinned at the servo setpoint on a bright scene), a sustained-level-step event log, and the trend graph the temperature fit reads. On the device itself: a ZERO button on the TOF SENSOR screen, **Z/T tags** on the main screen whenever a zero offset or temperature correction is being applied (the DISTANCE readout is now in the same frame as the FOV beside it), a red **TOF STALL** tag, a live shot counter and a **STACK DONE** banner with a backlight blink when a stack finishes, a two-tap **REBOOT** and uptime/fault counters on MEMORY & INFO, and a boot notice when a calibration was auto-restored or reset. Under the hood the serial port moved to the ESP32-S3's hardware USB-CDC (the unused TinyUSB classes cost 45 KB of flash and 31 KB of RAM), cold data rings moved to PSRAM, `/tofdbg` streams from one PSRAM buffer, telemetry is built on the stack and shared across clients, and the dashboard is served with an ETag so reloads are free. The build pipeline gained terser + zopfli (−15 KB of gzip), a JS syntax gate that refuses to embed a broken dashboard, path-based version counting, and a release script that tags the commit it actually shipped.
...
* **Stack Vibration Report (V12.5):** Per-frame vibration history of the last stack — during-exposure blur per photo (same DSP as the analyzer's broadband row), flagged against an adjustable pixel-blur threshold, with a full-stack V/H timeline, click-to-locate frame markers, copyable cull list, saved-report archive (last 8, browser-side), and the stack-summary PNG card available any time.
* **Field Resilience (V12.4):** Boot-loop auto-rollback, reset-reason diagnostics ring surfaced on the dashboard, loop/sensor watchdog with I²C self-heal, NVS calibration backup with auto-restore, and captive-portal fallback on a wrong WiFi password — built for unattended devices with no serial access.
