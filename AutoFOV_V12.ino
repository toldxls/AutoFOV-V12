...
//
// V12.6 role swap (v12.5.50): the PRIMARY (hi=false) config is now the centre
// 8×8 ROI at the 200 ms max budget — ~¼ the SPADs cuts the collected signal
// toward the linear regime (this scene parks at the part's max-signal
// operating point) and halves the sensed patch on the bellows (~3 mm spot at
// 20 mm — a feature: less scene averaging). The hi=true branch is no longer a
// user mode (web toggle removed v12.5.43) — it survives as the re-lock cure's
// flip target, deliberately maximally different (full array + 8 ms) so the
// change-and-back that clears the cold-start state still changes both knobs.
// A TFT hiRefl toggle, if used, now selects the full-array/8 ms config.
void applyReflConfig(bool hi) {
  ...
  if (hi) {
    ...
    // 200 ms (L4 driver max) — with histogram merging disabled (v12.5.49)
    // the deep single-histogram integration replaces the merge ring's 6×
    // depth. ~5 Hz update; EMA/averaging timescales stretch accordingly.
...
// logToStack=false publishes the scalars but skips the per-stack settle log —
// used for frame 1, whose window is pre-stack bench activity, not a motor-move
// settle (the old code excluded it by accident of ordering; now explicit).
static void vibAnalyzeSettle(uint32_t startPos, bool logToStack) {
  ...
  if (logToStack && vibSettleLogCount < VIB_SETTLE_LOG_MAX)
...
void vibTask(void *pvParameters) {
  ...
  uint32_t envHopCnt = 0;          // V12.6: hops since stack start (decimation phase)
  ...
  // V12.6: consume a pending stack-start seq — the full per-stack reset. A
  // lambda because TWO sites must run it: the regular start handler below, and
  // the pulse handler when frame 1's pulse arrives in the same iteration whose
  // earlier start check raced past both Core-1 bumps (start-seq lands before
  // pulse-seq in loop() program order, but this task loads them µs apart —
  // without the re-check the pulse would be judged against the frozen previous
  // stack's state and frame 1 silently dropped).
  auto consumeStackStart = [&]() {
    uint32_t ss = vibStackStartSeq.load(std::memory_order_acquire);
    if (ss == lastStartSeen) return;
    lastStartSeen = ss;
    vibSettleLogCount = 0;
    inStack       = true;
    ...
    // V12.6: reset the deep history for the new stack. The frames memset means
    // an index the pulse handler never reaches (coalesced pulses) can only
    // read as zeros, never as the previous stack's data.
    if (vibHistReady) {
      memset(vibHistFrames, 0, (size_t)VIB_HIST_FRAMES_MAX * sizeof(VibFrameRec));
  ...
  for (;;) {
    ...
    if (ps != lastPulseSeen) {
      ...
      // Frame 1's start bump precedes its pulse bump on Core 1, so if a stack
      // pulse arrives while the history is not recording, the start seq is
      // already visible — consume it now instead of next iteration.
      if (fn > 0 && vibHistState.load(std::memory_order_relaxed) != 1)
        consumeStackStart();
      uint32_t startPos = (vibPulseRingPos + VIB_RAW_LEN - VIB_SETTLE_SAMPLES) % VIB_RAW_LEN;
      // Frame 1's window is pre-stack bench activity — publish its scalars but
      // keep it out of the per-stack suggested-wait pool.
      vibAnalyzeSettle(startPos, fn != 1);
      ...
      if (fn > 0 && vibHistReady &&
        ...
        if (fn <= VIB_HIST_FRAMES_MAX) {
          ...
          // Coalesced pulses (two edges inside one task pass — chatter, or a
          // RUN TEST tap overwriting the payload) skip frame numbers; mark the
          // never-seen records explicitly unmeasured (flags 0) so the report
          // skips them instead of showing zeroed rows.
          for (uint32_t g = fc; g + 1 < fn; g++) {
            vibHistFrames[g].tMs = 0;
            vibHistFrames[g].blurVc = 0xFFFF; vibHistFrames[g].blurHc = 0xFFFF;
            vibHistFrames[g].settleMs = 0;    vibHistFrames[g].flags  = 0;
          }
...
void setup() {
  ...
  if (xSemaphoreTake(i2cMutex, portMAX_DELAY)) {
    ...
    //  - v12.5.49: merge OFF — no state machinery at all, one fixed
    //    processing path — and the lost 6× integration bought back with a
    //    33→200 ms timing budget (the L4 driver max) in applyReflConfig's
    //    primary branch: single histograms as deep as the old merged ones.
    //    ~5 Hz update, fine for hand-adjusted bellows. 1.4 h bench: level
    //    σ 0.40, remaining wobble = the intrinsic A/B phase pair (50/50 by
    //    construction — no occupancy to drift).
    //  - v12.5.50: primary ROI 16×16 → centre 8×8 (~¼ signal, toward the
    //    linear regime; ~3 mm sensed spot). NOTE: reported level shifts with
    //    any pipeline change → FOV recalibration once stable.
...
void loop() {
  ...
  if (tofRelockPhase == TOF_RELOCK_DWELL) {
    ...
    } else if (currentMode == CAL_SAMPLING || tofSampleActive || isSequenceActive) {
      // Never glitch a capture — device cal window, web temp-cal 2 s average,
      // or an active shutter sequence (the vib report stamps FOV at stack
      // end). The pending fire retries every pass until the way is clear.
    } else if (xSemaphoreTake(i2cMutex, pdMS_TO_TICKS(200))) {
      sensor.VL53L4CX_StopMeasurement();
  ...
  if (tofSampleActive) {
    // Mid-re-lock the sensor ranges in the OPPOSITE ROI/timing — those reads
    // would poison the average, so they don't accumulate. (A new sample can't
    // start a cycle — the fire branch above holds while tofSampleActive — but
    // the command can land while a dwell is already running.)
    uint32_t t = (tofRelockPhase == TOF_RELOCK_IDLE)
                     ? sensorDistTenths.load(std::memory_order_acquire) : 0;
    if (t >> 31) {
